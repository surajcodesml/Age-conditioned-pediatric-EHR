...
def paired_compare(adkm_df: pd.DataFrame, nint_df: pd.DataFrame, metric: str = "recall@5") -> dict:
    ...
    a = a.loc[common]
    b = b.loc[common]
    delta_win = a[metric].to_numpy(dtype=float) - b[metric].to_numpy(dtype=float)
    ...
    for _ in range(500):
        ...
        # nanmean: empty-label windows can yield NaN recall@K
        boots.append(float(np.nanmean(delta_win[mask])) if mask.any() else 0.0)
    return {
        "metric": metric,
        "delta_point": float(np.nanmean(delta_win)),
